...
public class Puzzle {
	...
	public int computeH1(Node currentState) {
		...
		for (int i = 0; i < MAX_ROW; i++) {
			for (int j = 0; j < MAX_COL; j++) {
				if (currentState.getTile(i, j) != 0 && currentState.getTile(i, j) != goalState.getTile(i, j))
					output++;
			}
		}
		return output;
	...
	// Using manhattanDistance above to compute H2
	public int computeH2(Node currentState) {
		int result = 0;
		for (int i = 1; i <= 8; i++) {
			result += PuzzleUtils.manhattanDistance(currentState.getLocation(i), goalState.getLocation(i));
		}
		return result;
	}
	...
	public Node moveWhiteTile(Node currentState, char operator) {
		...
		if (operator == 'u') {// Case-1: Move tile UP
			...
			// New position of tile if move DOWN
			int row = whiteTile[0] + 1;
			int col = whiteTile[1];
			...
			// New position of tile if move LEFT
			int row = whiteTile[0];
			...
			// New position of tile if move RIGHT
			int row = whiteTile[0];
			int col = whiteTile[1] + 1;
